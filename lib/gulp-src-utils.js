...
module.exports = {
  gulpify: gulpifySrc,
  pipeSortFactory: pipeSortFactory,
  watch: watchSrc
};
 ...
 * @returns {Function} A function which takes a file stream as argument and returns a sorted version of that stream.
 *
 * @example
 * var pipeSort = pipeSortFactory(sortStylesheets);
 * pipeSort(gulp.src('*.css'))
 *  .pipe(gulp.dest('build'));
 */
function pipeSortFactory(compare) {
  return function(src) {
...
function gulpifySrc(src, options) {
  ...
  if (_.isFunction(src.pipe)) {
    ...
    // Source if a function which will return a file stream.
    gulpSrc = src(options);
  } else {
    ...
    if (_.isString(src) || _.isArray(src)) {
      ...
      _.defaults(gulpOptions, _.omit(src, 'files'));
    } else {
  ...
  if (_.isFunction(compare)) {
    gulpSrc = pipeSortFactory(compare)(gulpSrc);
  }
...
function getSrcOptions(src) {
  return _.isObject(src) ? _.pick(src, 'base', 'cwd') : {};
}
