 ...
 */
function processedFiles(to, toExt, toDir) {
  return through.obj(function(file, enc, callback) {

    to = _.isFunction(to) ? to() : to;
    toExt = _.isFunction(toExt) ? toExt() : toExt;
    toDir = _.isFunction(toDir) ? toDir() : toDir;

    var base = file.base,
        relativeBase = path.relative(root, base),
        relativePath = path.relative(base, file.path),
        fromExt = path.basename(relativePath).replace(/.*\./, ''),
        fromPath = path.join(relativeBase, relativePath),
        relativeTo = path.relative(root, to),
        toPath = path.normalize(path.join(relativeTo, toDir || '.', relativePath));
